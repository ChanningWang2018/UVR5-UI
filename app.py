...
def save_current_settings(*values):
    ...
    try:
        ...
        gr.Info(
            i18n("Current settings saved successfully! They will be loaded next time")
        )
    except Exception as e:
    ...
    try:
        ...
        for section_name, section_comps in components.items():
            for comp_name, comp_instance in section_comps.items():
                default_value = default_settings.get(section_name, {}).get(
                    comp_name, None
                )

                if (
                    isinstance(comp_instance, gr.Dropdown)
                    and hasattr(comp_instance, "choices")
                    and default_value is not None
                ):
                    if default_value not in comp_instance.choices:
                        print(
                            f"Warning: Default value '{default_value}' for '{comp_name}' not in choices {comp_instance.choices}. Setting to None."
                        )
                        default_value = None
        ...
        gr.Info(
            i18n("Settings reset to default. Default settings will be loaded next time")
        )
        return updates
...
with gr.Blocks(
    theme=loadThemes.load_json() or "NoCrypt/miku", title="🎵 UVR5 UI 🎵", js=js_func
) as app:
    ...
    gr.Markdown(
        i18n(
            "Try UVR5 UI on Hugging Face with A100 [here](https://huggingface.co/spaces/TheStinger/UVR5_UI)"
        )
    )
    ...
    with gr.Tabs():
        with gr.TabItem("BS/Mel Roformer"):
            with gr.Row():
                ...
                with gr.Row():
                    with gr.Column():
                        roformer_all_models = gr.Button(
                            i18n("All models"), variant="primary"
                        )
                        roformer_downloaded_models = gr.Button(
                            i18n("Downloaded models only"), variant="primary"
                        )
                    roformer_output_format = gr.Dropdown(
                        label=i18n("Select the output format"),
                        choices=output_format,
                        value=initial_settings.get("Roformer", {}).get(
                            "output_format", None
                        ),
                        interactive=True,
                    )
                ...
                roformer_all_models.click(
                    fn=get_all_roformer_models, inputs=[], outputs=[roformer_model]
                )
            ...
            with gr.Row():
                roformer_audio = gr.Audio(
                    label=i18n("Input audio"), type="filepath", interactive=True
                )
            ...
            with gr.Accordion(i18n("Batch separation"), open=False):
                ...
                with gr.Row():
                    roformer_info = gr.Textbox(
                        label=i18n("Output information"), lines=5, interactive=False
                    )
            ...
            roformer_model.change(
                roformer_update_stems,
                inputs=[roformer_model],
                outputs=roformer_stem_row4,
            )
            roformer_model.change(
                roformer_update_stems,
                inputs=[roformer_model],
                outputs=roformer_stem_row6,
            )
        ...
        with gr.TabItem("MDX23C"):
            with gr.Row():
                ...
                with gr.Row():
                    ...
                    mdx23c_output_format = gr.Dropdown(
                        label=i18n("Select the output format"),
                        choices=output_format,
                        value=initial_settings.get("MDX23C", {}).get(
                            "output_format", None
                        ),
                        interactive=True,
                    )
                ...
                mdx23c_all_models.click(
                    fn=get_all_mdx23c_models, inputs=[], outputs=[mdx23c_model]
                )
                mdx23c_downloaded_models.click(
                    fn=get_downloaded_mdx23c_models, inputs=[], outputs=[mdx23c_model]
                )
            with gr.Accordion(i18n("Advanced settings"), open=False):
                with gr.Group():
                    with gr.Row():
            ...
            mdx23c_model.change(
                update_stems, inputs=[mdx23c_model], outputs=mdx23c_stem_row4
            )
            mdx23c_model.change(
                update_stems, inputs=[mdx23c_model], outputs=mdx23c_stem_row6
            )
        ...
        with gr.TabItem("MDX-NET"):
            with gr.Row():
                ...
                mdxnet_all_models.click(
                    fn=get_all_mdxnet_models, inputs=[], outputs=[mdxnet_model]
                )
                mdxnet_downloaded_models.click(
                    fn=get_downloaded_mdxnet_models, inputs=[], outputs=[mdxnet_model]
                )
            with gr.Accordion(i18n("Advanced settings"), open=False):
                with gr.Group():
                    with gr.Row():
        ...
        with gr.TabItem("VR ARCH"):
            with gr.Row():
                ...
                vrarch_all_models.click(
                    fn=get_all_vrarch_models, inputs=[], outputs=[vrarch_model]
                )
                vrarch_downloaded_models.click(
                    fn=get_downloaded_vrarch_models, inputs=[], outputs=[vrarch_model]
                )
            with gr.Accordion(i18n("Advanced settings"), open=False):
                with gr.Group():
                    with gr.Row():
        ...
        with gr.TabItem(i18n("Themes")):
            ...
            mode_select = gr.Dropdown(
                label=i18n("Mode"),
                info=i18n(
                    "Select the mode you want to use. (Requires restarting the App)"
                ),
                choices=["light", "dark"],
            ...
            themes_select.change(
                fn=loadThemes.select_theme, inputs=themes_select, outputs=[]
            )
            mode_select.change(
                fn=loadThemes.select_mode, inputs=mode_select, outputs=[]
            )
        ...
        with gr.TabItem(i18n("Settings")):
            with gr.Accordion(i18n("Language selector"), open=False):
                selected_language = gr.Dropdown(
                    label=i18n("Language"),
                    info=i18n(
                        "Select the language you want to use. (Requires restarting the App)"
                    ),
                    value=get_language_settings(),
                    choices=["Language automatically detected by system"]
                    + i18n._get_available_languages(),
                    interactive=True,
                )

                selected_language.change(
                    fn=save_lang_settings, inputs=[selected_language], outputs=[]
                )
            with gr.Accordion(i18n("Alternative model downloader"), open=False):
                download_method = gr.Dropdown(
                    label=i18n("Download method"),
                    info=i18n(
                        "Select the download method you want to use. (Must have it installed)"
                    ),
                    value=lambda: None,
                    choices=["wget", "curl"],
                    interactive=True,
                ...
                download_source = gr.Dropdown(
                    ...
                    info=i18n(
                        "Select the source to download the model from (Github or Hugging Face)"
                    ),
                    value=lambda: None,
                ...
                alternative_model_downloader_button = gr.Button(
                    i18n("Download!"), variant="primary"
                )
                ...
                alternative_model_downloader_button.click(
                    alternative_model_downloader,
                    [download_method, model_key_json, download_source],
                    [alternative_model_downloader_output],
                )

            with gr.Accordion(i18n("Separation settings management"), open=False):
                gr.Markdown(
                    i18n(
                        "Save your current separation parameter settings or reset them to the application defaults"
                    )
                )
                with gr.Row():
                    save_settings_button = gr.Button(
                        i18n("Save current settings"), variant="primary"
                    )
                    reset_settings_button = gr.Button(
                        i18n("Reset settings to default"), variant="primary"
                    )

                save_settings_button.click(
                    save_current_settings, all_configurable_inputs, None
                )
                reset_settings_button.click(
                    reset_settings_to_default, None, all_configurable_inputs
                )
